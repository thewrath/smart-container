...
class Container {
  ...
  load(rootDir, configPath) {
    ...
    if (this.configuration.services) {
      for (const key of Object.keys(this.configuration.services)) {
        ...
        this.register(key, require(filePath), isSingleton)
  ...
  constructArguments(argumentsList) {
    ...
    for (const arg of argumentsList) {
      if (typeof arg === 'string' && arg.startsWith('@')) {  /* -> Check if the argument is a service reference */
        const serviceName = arg.substr(1);
        args.push(this.get(serviceName));
  ...
  register(name, serviceClass, isSingleton) {
    ...
    this.serviceDefinitions[name] = new ServiceDefinition(name, serviceClass, isSingleton);
    return this.serviceDefinitions[name];
  }

  /**
   * @function constructAllServices
   * @description Build all services registered in this.serviceDefinitions, usefull when you want to loop and call all services 
   * @throws {Error}
   */
  constructAllServices() {
    for (let serviceDefinition in this.serviceDefinitions) {
   ...
   * @description Return all contsructed services 
   * @returns {Array}
   */
  getServices() {
